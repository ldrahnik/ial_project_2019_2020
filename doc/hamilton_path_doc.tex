...
K naměření dat byly použity grafy s počtem vrcholů od 3 do 13 uvedené ve složce `time\_complexity/graphs`. Všechny tyto grafy obsahují hrany propojující každý vrchol se všemi ostatními.
...
]{./../time_complexity/brute_force_info.txt}
...
\addplot table [x=vertices_count,y=explored_vertices_count] {./../time_complexity/brute_force_info.txt};
\end{axis}
\end{tikzpicture}
\end{figure}
...
\addplot table [x=vertices_count,y=executed_time] {./../time_complexity/brute_force_info.txt};
\end{axis}
\end{tikzpicture}
\end{figure}
